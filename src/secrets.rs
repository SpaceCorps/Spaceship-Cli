//! Where API keys and secrets live: the OS keystore, one entry per account under `account:{name}`.
//!
//! DPAPI on Windows, the Keychain on macOS, libsecret on Linux. When none is available the tool
//! refuses to start rather than silently writing a file - `SPACESHIP_ALLOW_PLAINTEXT_STORE=1` is
...
const SERVICE: &str = "spaceship-cli";
...
/// The backend for this process. Cached: changing `SPACESHIP_SECRET_STORE` mid-process has no
...
fn build() -> Result<Store> {
    if let Some(forced) = std::env::var("SPACESHIP_SECRET_STORE").ok().filter(|s| !s.trim().is_empty()) {
        return match forced.to_lowercase().as_str() {
            ...
            _ => Err(Error::invalid(format!(
                "SPACESHIP_SECRET_STORE='{forced}' is not a backend available on this platform."
            ))
            .fix("Unset SPACESHIP_SECRET_STORE, or set it to one of: dpapi, keychain, libsecret, plaintext.")),
        };
    }
...
fn fallback(reason: &str) -> Result<Store> {
    if std::env::var("SPACESHIP_ALLOW_PLAINTEXT_STORE").as_deref() == Ok("1") {
        return Ok(Store::Plaintext);
    }
    Err(Error::other("No secure credential store is available on this machine.").detail(reason).fix(
        "Install a keystore, or set SPACESHIP_ALLOW_PLAINTEXT_STORE=1 to store credentials in a 0600 file instead.",
    ))
}
...
impl Store {
    ...
    pub fn get(self, key: &str) -> Result<Option<SecretEntry>> {
        match self {
            Store::Keychain => {
    ...
    pub fn set(self, key: &str, creds: &SecretEntry) -> Result<()> {
        let value = serde_json::to_string(creds).expect("SecretEntry serializes");
        match self {
            Store::Keychain => {
                let (ok, _, err) =
...
fn parse_secret_entry(raw: &str) -> Result<Option<SecretEntry>> {
    match serde_json::from_str::<SecretEntry>(raw) {
        Ok(entry) => Ok(Some(entry)),
        Err(_) => {
            // Also support colon-separated fallback: key:secret
            if let Some((k, s)) = raw.split_once(':') {
                Ok(Some(SecretEntry { api_key: k.to_string(), api_secret: s.to_string() }))
            } else {
                Err(Error::other("Failed to parse stored credential entry."))
            }
        }
    }
}
...
fn warn_plaintext() {
    ...
    WARNED.get_or_init(|| {
        eprintln!(
            "warning: API credentials are stored unencrypted in {} (SPACESHIP_ALLOW_PLAINTEXT_STORE=1).",
...
mod dpapi {
    ...
    pub fn unprotect(data: &[u8], path: &Path) -> Result<Vec<u8>> {
        ...
        if ok == 0 {
            return Err(Error::other("The stored API credentials could not be decrypted.")
                .detail(format!(
                    "DPAPI failed: {}. This happens when the file was written by a different Windows user or on a different machine.",
                    std::io::Error::last_os_error()
                ))
                .fix(format!("Delete {} and run: spaceship accounts add <name>", path.display())));
        }
        Ok(take(out))
    }
}
