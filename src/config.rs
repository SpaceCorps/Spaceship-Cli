//! Non-secret account metadata in a readable YAML file. The API credentials themselves never
...
pub struct AccountConfig {
    /// A short label identifying this account (or key prefix).
    pub identity: String,
    pub added_at: String,
}
...
pub fn config_dir() -> PathBuf {
    if let Some(dir) = std::env::var_os("SPACESHIP_CONFIG_DIR").filter(|v| !v.is_empty()) {
        return PathBuf::from(dir);
    }

    #[cfg(windows)]
    let dir = std::env::var_os("APPDATA").map(PathBuf::from).unwrap_or_default().join("spaceship-cli");

    #[cfg(not(windows))]
    let dir = {
        let home = std::env::var_os("HOME").map(PathBuf::from).unwrap_or_default();
        if cfg!(target_os = "macos") {
            home.join("Library").join("Application Support").join("spaceship-cli")
        } else {
...
pub fn load() -> Result<Config> {
    ...
    serde_norway::from_str::<Option<Config>>(&text).map(Option::unwrap_or_default).map_err(|e| {
        Error::other(format!("{} is not valid.", path.display()))
            .detail(e.to_string())
            .fix(format!("Fix or delete {}, then run: spaceship accounts add <name>", path.display()))
    })
}
        ...
        match file.try_lock() {
            ...
            Err(fs::TryLockError::WouldBlock) => {
                return Err(Error::other(format!(
                    "Timed out waiting for the lock at {}. Another spaceship process may be stuck.",
                    path.display()
                )));
            }
...
mod tests {
    ...
    fn config_round_trip() {
        ...
        c.accounts.insert("work".into(), AccountConfig { identity: "key_xxx".into(), added_at: now_utc() });
        let yaml = serde_norway::to_string(&c).unwrap();
        let loaded: Config = serde_norway::from_str(&yaml).unwrap();
        assert_eq!(loaded.accounts["work"].identity, "key_xxx");
    }
}
